...
def send_to_arduino(command):
    ...
    time.sleep(.25)  # Give Arduino time to process the command
...
while not game_over:
    ...
    if turn == PLAYER_TURN and not game_over and not_over:
        draw_board(board)  # Update the board visually
        valid_move = False
        while not valid_move:
            try:
                col = (int(input("Player 1, type your column (1-7): ")) - 1)
                if col in range(0, 7) and is_valid_location(board, col):
                    valid_move = True
                else:
    ...
    if turn == AI_TURN and not game_over and not_over:
        ...
        print(f"Robot chooses column: {col+1}")
        
        send_to_arduino(f"MOVE_TO {col}")  # Command robot to position
        input("Press Enter after the robot has placed the puck.")  # Wait for confirmation
        send_to_arduino("RESET")  # Command robot to return to rest position
